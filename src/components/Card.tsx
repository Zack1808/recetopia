...
const Card: React.FC<CardProps> = ({ user, title, image, id }) => {
  return (
    <Link
      to={`/recipes/${id}`}
      className={`relative flex min-h-96 rounded p-5 bg-cover   items-end overflow-hidden -z-10`}
    >
      <div
        className={`bg-[url("${image}")] absolute top-0 bottom-0 bg-center left-0 right-0 bg-cover hover:scale-105 transition-transform duration-500 -z-10`}
      ></div>
